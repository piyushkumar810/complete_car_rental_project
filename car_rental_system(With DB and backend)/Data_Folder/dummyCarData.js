...
for (let i = 0; i < 100; i++) {
  ...
  const car = {
    ...
    fuelEfficiency: faker.datatype.float({ min: 10, max: 25, precision: 0.01 }), // km/l
    numberOfSeats: faker.datatype.number({ min: 2, max: 8 }), // Units (number of seats)
    color: faker.commerce.color(),
    dailyRentalRate: faker.datatype.number({ min: 500, max: 2000 }), // Rupees
    weeklyRentalRate: faker.datatype.number({ min: 3000, max: 14000 }), // Rupees
    monthlyRentalRate: faker.datatype.number({ min: 12000, max: 60000 }), // Rupees
    ...
    currentMileage: faker.datatype.number({ min: 0, max: 100 }), // km
